...
router.get('/dashboard', (req, res) => {
  ...
  ['revisions', 'calls', 'admin', 'scope'].forEach(cat => {
    ...
    lossBreakdown[cat] = {
      ...
      loss: Math.round(hrs * settings.minimumRate)
    };
  });

  // Earnings trend (daily for last 30 days)
  const earningsTrend = [];
  for (let i = 29; i >= 0; i--) {
    const date = new Date();
  ...
  if (effectiveRate < settings.minimumRate) {
    alerts.push({ type: 'danger', message: `Effective rate (${settings.currency}${effectiveRate}/hr) is below your minimum rate (${settings.currency}${settings.minimumRate}/hr)` });
  }
  ...
  if (revisionPercent > 20) {
    alerts.push({ type: 'warning', message: `${revisionPercent}% of your time is spent on revisions — eating into profits` });
  }

  if (nonBillablePercent > 40) {
    alerts.push({ type: 'danger', message: `${nonBillablePercent}% of work is non-billable — too much unpaid work` });
  }

  // Per-project scope creep alerts
  projects.filter(p => p.status === 'active').forEach(p => {
    ...
    if (p.estimatedHours > 0 && pHours > p.estimatedHours * 1.1) {
      ...
      alerts.push({ type: 'warning', message: `"${p.name}" has ${creep}% scope creep — actual hours exceed estimate` });
    }
  });
...
router.get('/portfolio', (req, res) => {
  ...
  projects.forEach(p => {
    ...
    clientStats[p.clientName].projects.push({
      id: p.id,
      name: p.name,
      value,
  ...
  // Calculate avg effective rate per client
  Object.values(clientStats).forEach(client => {
    client.avgEffectiveRate = client.totalHours > 0 ? Math.round(client.totalValue / client.totalHours) : 0;
    client.totalHours = Math.round(client.totalHours * 10) / 10;
    client.health = client.avgEffectiveRate >= settings.minimumRate * 1.3 ? 'profitable' :
                    client.avgEffectiveRate >= settings.minimumRate ? 'risky' : 'loss';
  });
  ...
  res.json({
    totals: { totalValue, totalHours: Math.round(totalHours * 10) / 10, totalLoss, avgEffectiveRate },
    clients,
    bestClient: clients[0] || null,
    worstClient: clients[clients.length - 1] || null,
    currency: settings.currency
  });
});
