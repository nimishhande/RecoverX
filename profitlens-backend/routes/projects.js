...
router.get('/:id', (req, res) => {
  ...
  res.json({
    ...project, totalHours: Math.round(totalHours * 10) / 10,
    billableHours: Math.round(billableHours * 10) / 10,
    nonBillableHours: Math.round(nonBillableHours * 10) / 10,
    nonBillablePercent, effectiveRate, hiddenLoss, scopeCreep, health, totalValue, categoryBreakdown, logs
  });
});

router.post('/', (req, res) => {
  const { name, clientName, projectType, fixedPrice, hourlyRate, estimatedHours, minimumRate } = req.body;
  if (!name || !clientName) return res.status(400).json({ error: 'Name and client name required' });
  const project = {
    id: uuidv4(), name, clientName, projectType: projectType || 'fixed',
    fixedPrice: fixedPrice || null, hourlyRate: hourlyRate || null,
    estimatedHours: estimatedHours || 0, minimumRate: minimumRate || store.settings.minimumRate,
